...
func InitRedis()  {
	RedisPool = redis.Pool{
		...
		Dial: func() (redis.Conn, error) {
			return redis.Dial("tcp", "192.168.28.128:6379")
		},
	}
}
...
func Login(mobile, pwd string) (string, error) {
	...
	pwd_hash := pwd
	err := GlobalConn.Where("mobile = ?", mobile).Select("name").Where("password_hash = ?", pwd_hash).Find(&user).Error
...
func SaveRealName(userName,realName,idCard string) error{
	return GlobalConn.Model(new(User)).Where("name=?",userName).Updates(map[string]interface{}{"real_name":realName,"id_card":idCard}).Error
}